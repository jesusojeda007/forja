...
export async function renderMejoras(
  ...
  const banner = flash?.found !== undefined
    ? `<div style="border:1px solid var(--ok);background:rgba(21,128,61,.08);color:var(--ok);padding:10px 14px;font-size:12.5px;margin-bottom:16px">Búsqueda completada: ${esc(flash.found)} ${flash.found === "1" ? "mejora nueva propuesta" : "mejoras nuevas propuestas"}.</div>`
    : flash?.applied
      ? `<div style="border:1px solid var(--ok);background:rgba(21,128,61,.08);color:var(--ok);padding:10px 14px;font-size:12.5px;margin-bottom:16px">✓ Mejora aplicada — activa desde el siguiente mensaje.</div>`
      : flash?.dismissed
