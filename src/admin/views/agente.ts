...
function nodeHtml(n: NodeSpec): string {
  ...
  <div class="node-card absolute cursor-pointer"
       style="left:${n.x}px;top:${n.y}px;width:${n.w}px;padding:11px 13px;${
         n.on
           ? "background:linear-gradient(135deg, rgba(21,128,61,.10), rgba(21,128,61,.04)), var(--panel2);border:1px solid var(--ok);box-shadow:0 0 0 1px rgba(21,128,61,.25), 0 0 18px -6px rgba(21,128,61,.35);"
    ...
    <div class="flex items-center gap-2">
      <span class="w-[22px] h-[22px] flex-none flex items-center justify-center" style="border:1px solid ${n.accent};background:${n.on ? "rgba(21,128,61,.14)" : "var(--panel2)"}">
      ...
      ${n.on ? `<span class="ml-auto text-[8.5px] tracking-[.1em] font-semibold" style="color:var(--ok);border:1px solid var(--ok);padding:0 4px;background:rgba(21,128,61,.10)">● ACTIVO</span>` : ""}
    </div>
...
const SAVED_BANNER = `<div class="px-3 py-2 text-[12.5px] mb-4" style="border:1px solid var(--ok);background:rgba(21,128,61,.08);color:var(--ok)">✓ Guardado — aplica desde el siguiente mensaje.</div>`;
...
const SAVE_BTN = `<button type="submit" class="bigbtn font-display font-bold text-[12.5px] cursor-pointer" style="background:var(--accent);border:1px solid var(--accent);color:#ffffff;box-shadow:3px 3px 0 var(--linelit);padding:8px 16px">Guardar</button>`;
    ...
    return modalShell("cpu", "Agente (el cerebro)", badge, `
      ...
      <form hx-post="/admin/agente/node/brain/save" hx-target="#modal-root" hx-swap="innerHTML" class="mb-5">
        ...
        <button type="submit" class="${d.cfg.botPaused ? "bigbtn font-display font-bold" : "ghostbtn"} text-[12.5px] cursor-pointer inline-flex items-center gap-2"
                style="${d.cfg.botPaused
                  ? "background:var(--accent);border:1px solid var(--accent);color:#ffffff;box-shadow:3px 3px 0 var(--linelit);padding:9px 16px"
      ...
      <form hx-post="/admin/agente/node/brain/save" hx-target="#modal-root" hx-swap="innerHTML">
        ...
        <div class="flex flex-wrap gap-2 mt-3">
          <button type="submit" class="bigbtn font-display font-bold text-[12.5px] cursor-pointer" style="background:var(--accent);border:1px solid var(--accent);color:#ffffff;box-shadow:3px 3px 0 var(--linelit);padding:8px 16px">Guardar prompt manual</button>
